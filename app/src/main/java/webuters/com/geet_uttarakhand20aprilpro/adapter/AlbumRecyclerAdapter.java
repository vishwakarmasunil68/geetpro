...
import webuters.com.geet_uttarakhand20aprilpro.activity.NewAlbumSongsActivity;
...
public class AlbumRecyclerAdapter extends RecyclerView.Adapter<AlbumRecyclerAdapter.ViewHolder> {
    private List<AlbumPOJO> items;
    Activity activity;
    Fragment fragment;

    public AlbumRecyclerAdapter(Activity activity, Fragment fragment, List<AlbumPOJO> items) {
        this.items = items;
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        Picasso.with(activity)
                .load(items.get(position).getAlbumCover())
                .into(holder.iv_album);

        holder.tv_album.setText(items.get(position).getAlbumName());
...
//                Intent i = new Intent(activity, AlbumeSongs.class);
                ...
                i.putExtra("SongImage123", items.get(position).getAlbumCover());
                i.putExtra("album_name", items.get(position).getAlbumName());
